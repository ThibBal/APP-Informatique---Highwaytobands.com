...
{
	global $bdd;
	$req = $bdd->prepare('SELECT login FROM membre INNER JOIN forum_sujet ON membre.id=forum_sujet.id_user WHERE forum_sujet.actif=0 AND forum_sujet.id_sujet= ?');
	$req->execute(array($donnees));
	return $req;
}
...
{
    global $bdd;
    $req = $bdd->prepare('SELECT * FROM membre INNER JOIN forum_message ON membre.id=forum_message.id_user WHERE forum_message.actif=0 AND id_message= ?');
    $req->execute(array($donnees));
    return $req;
}
