...
if (isset($_POST['nouvtopic'])) {
    ...
    $nouvtopic = addslashes($nouvtopic);
...
if (isset($_POST['nouvmessage'])) {
    ...
    $nouvmessage = addslashes($nouvmessage);
    include '../structure/connexion_bdd.php';
...
if (isset($_POST['topicforum'])) {
    ...
    if (isset($_SESSION['pseudo']) && $_POST['statuttopic']!="1") {
        ...
        echo '">'.'<script type="text/javascript" src="../structure/ckeditor/ckeditor.js"></script>'
                .'<script type="text/javascript">CKEDITOR.replace("nouvtopic");</script>'
        . '<input type="submit" value="Valider"></form></td></tr>';
	...
	echo '</div>';
        ...
        if (isset($_SESSION['id'])){
			...
			echo "onclick=\"if (window.confirm('Etes vous sur de vouloir supprimer cela?')){location.href='default.htm';return true;} else {return false;}\"";
			echo '></form> ';
    ...
    if (isset($_SESSION['pseudo'])) {
        echo '<tr class="nouvpost"><td>';
        echo $_SESSION['pseudo'];
        echo '</td><td>Maintenant</td><td><form method="post">'
            . '<textarea name="nouvtopic" id="nouvtopic">Nouveau Topic</textarea>'
                . '<input type="hidden" name="catforum" value="'.$_POST['catforum'].'">';
